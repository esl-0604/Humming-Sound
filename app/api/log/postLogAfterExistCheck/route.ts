...
export async function POST(req: NextRequest) {
  ...
  const checkIsExistingLog = await db.collection("EventLog").findOne(
    { logging_id: log.logging_id, session_id: log.session_id }, // 서버 삽입 시점의 시간
  );
  if (checkIsExistingLog) {
    return NextResponse.json("Existing Log", {
      status: 200,
      statusText: "Log Already Exists",
    });
  }
